...
而apply中遍历 middlewares 数组，逐个调用 middleware(middlewareAPI)，无非是为了获取中间件的内层函数。
...
讲师回复： 这样做是为了避免在接下来中间件链的串联过程中，dispatch被调用。error对象中”Dispatching while constructing your middleware is not allowed.“直译过来就是”在构建中间件的过程中，dispatch是不被允许的“。
