...
![示例](https://s0.lgstatic.com/i/image/M00/35/74/Ciqc1F8VZpKAVYWOAABLt08AfuQ883.png)
...
最后是 has 代理过程，当我们**判断属性是否存在于 instance.ctx 渲染上下文中时，就会进入 has 函数**，这个在平时项目中用的比较少，同样来举个例子，当执行 created 钩子函数中的 'msg' in this 时，就会触发 has 函数。
...
![示例](https://s0.lgstatic.com/i/image/M00/35/74/Ciqc1F8VZvaAYCgKAAHVSzimXjw614.png)

思考题目，在执行 setup 函数并获取结果的时候，我们使用 callWithErrorHandling 把 setup 包装了一层，它有哪些好处？

() get的时候回利用accessCache查找缓存，但是set的时候不会，这是为什么呢？
讲师回复： 因为这个 accessCache 缓存的是 key 对应的数据存储的类型，而不是具体存储的值，数据还是存储在 setupState、ctx、data、props 这些对象中

() 如果setup返回对象和data重名，我这边测试时会用data中的值来渲染，但是文中的源码确实先执行的setupContext中查找，3.0.1版本

() 在set里面判断`accessCache[key]`是对应的什么类型(比如`setupState`)，再`setupState[key]=value`为什么不行呢？
讲师回复： 理论上来说，set 的时候判断 accessCache 应该也是可以的
