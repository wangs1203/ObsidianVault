...
05讲述了在 Vue.js 3.0 中引入 reactive API，它可以把对象数据变成响应式，所以我们着重分析 reactive API 的实现原理，并学习了收集依赖的 get 函数， 这节课我们继续来分析 reactive API 中需要关注的另一个内容——派发通知的过程。
...
![示例](https://s0.lgstatic.com/i/image/M00/3A/99/CgqCHl8iOeqAJJlaAAHAhGDRoDQ714.png)
...
思考题目，为什么说 Vue.js 3 的响应式 API 实现和 Vue.js 2.x 相比性能要好，具体好在哪里呢？它又有哪些不足呢？欢迎你在留言区与我分享。
...
讲师回复： 类似 2.x 中的 targetStack，在 dep.js 中定义的

() 什么场景会用嵌套effect
讲师回复： 可以去看 Vue3 的单元测试: reactivity/__tests__/effect.spec.ts，搜 should allow nested effects
