...
() 我把一个组件componentA，当成参数（如：attr:Object.freeze({tag:componentA})）传递给组件componentB，参数componentB用动态组件component渲染 is直接指向attr.tag,这样有什么弊端不？
讲师回复： 可以传，不过为何要用参数传递呢，直接 import 这个 ComponentA 不可以吗
